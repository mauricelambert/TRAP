...
# sudo /usr/bin/perl -MCPAN -e'install File::Map'
...
TRAP (Tool for Regex Analysis with Perl) version 0.2.1
...
m~([0-9a-fA-F]{32}([0-9a-fA-F]{8})?([0-9a-fA-F]{24})?([0-9a-fA-F]{64})?|$[0-9]+[a-zA-Z]*$[0-9\w/\.]+$[\w/\.]+)~g
    ...
    process_match( $counter_ref, $files_ref, $filename, $phone_field, substr($&, 1, -1), @-,
        @+ )
      while ( $map =~
m$[^\d+-]((\+\d{2}(([ –-]|\xe2\x80\x93){1,3}\(?\d{2,6}\)?){3}(-\d{2})?)|(\(\d?\w{1,2}\d{2}\)([ -]#?\d{2,7}){1,4})(-\d)?|(\d{3,5}[ -]\d{6,8})(-\d{4})?|(\+?\d{0,2}[ -]?( ([–-]|\xe2\x80\x93))?((\d{2}|\(\d\))? ?)?((\d{3,4})[ \.-]){2}\d{2,4})|(\d{0,3}(\d{2}[ \.-]){4}\d{2})|(\+?(\(\d{1,2}\))?\(\d{1,3}\)[ -]?\d{2,4}[ -]\d{4})|(\d{4,5}(([ \.–-]|\xe2\x80\x93){1,3}\d{2}){3,4})|(\d{4} \d{4})|(\+\d{2}([ –-]|\xe2\x80\x93)?\(?\d\)?([\. –-]|\xe2\x80\x93){0,3}\d{1,4}(([\. –-]|\xe2\x80\x93){0,3}\d{2,4}){2,4})|(\d{3,5} ?\/ ?(\d{2,3} ?){3}\d?)|((\+?\d{1,2}[ -])?(\+ ?)?\d{6,15})|(\(\d{2}\)([ \.]\d{2,3}){3})|(\d( \d{2}){2}([ –-]|\xe2\x80\x93){3}\d( \d{2}){3}))[^\d+-]$g
      );
...
#pod     main;
#pod
#pod This function execute the script
